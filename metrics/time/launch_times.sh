...
run_workload() {
	...
	readarray -n 0 workload_result < <(isula run --cap-add SYSLOG --rm --runtime=${RUNTIME}  ${IMAGE} sh -c "$DATECMD $DMESGCMD")
...
EOF
	...
	if [ -n "$SCALING" ]; then
		...
		isula run --runtime=${RUNTIME} -d ${IMAGE} sh -c "tail -f /dev/null"
	fi
}
