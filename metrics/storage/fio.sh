...
TEST_NAME="fio"
IMAGE="registry.cn-hangzhou.aliyuncs.com/moyasu/local-fio"
...
init() {
	...
	isula run -d --rm --name="${CONTAINER_NAME}" --runtime=kata-runtime ${RUNTIME_EXTRA_ARGS} -v ${HOST_OUTPUT_DIRNAME}:${GUEST_OUTPUT_DIRNAME} -v ${HOST_INPUT_DIRNAME}:${GUEST_INPUT_DIRNAME} $IMAGE
}
...
run_test() {
	...
	if [ "${FIO_READTEST}" -eq 1 ]; then
		...
		local output=$(isula exec "${CONTAINER_NAME}" fio $fioopts ${GUEST_INPUT_DIRNAME}/$filename)
